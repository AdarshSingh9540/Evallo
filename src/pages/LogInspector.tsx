...
export function LogInspector() {
  ...
  const handleNewLog = useCallback((newLog: LogEntry) => {
    ...
    const matchesFilters = (log: LogEntry) => {
      ...
      if (filters.traceId && !log.traceId.toLowerCase().includes(filters.traceId.toLowerCase())) return false;
      return true;
    };
  ...
  const fetchLogs = async (currentFilters: LogFiltersType = filters, page: number = 1) => {
    setLoading(true);
    try {
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="space-y-2">
        ...
        <p className="text-slate-600">
          Search, filter, and analyze logs with advanced querying capabilities and real-time updates.
        </p>
      </div>
